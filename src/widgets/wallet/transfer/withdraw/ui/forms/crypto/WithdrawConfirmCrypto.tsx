...
const WithdrawConfirmCrypto: FC<IWithdrawConfirmCryptoProps> = ({
  ...
  const onConfirm = async (reSendCode = false) => {
    ...
    const confirmationCode = reSendCode
      ? null
      : stageReq.status === 2
      ? stageReq.code
      : input !== ""
          ...
          {localErrorInfoBox
            ...
            : stageReq.autoInnerTransfer && (
                <InfoBox icon={<IconApp size={30} code='t8' color='#ttt' />}>
                  The address is within our system. The transfer will be made via the internal network, and not through
                  the blockchain. Are you sure you want to continue?
                </InfoBox>
              )}
        </div>

        <Form wrapperClassName='w-full' onSubmit={() => onConfirm()}>
          ...
          <ConfirmButtons
            isConfirmDisabled={
              !!localErrorInfoBox || (input === "" && (stageReq.status === 0 || stageReq.status === 1))
            }
