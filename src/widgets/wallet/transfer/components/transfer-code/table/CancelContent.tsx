...
const CancelContent = ({ code, amount, confirm, currency, date = null }) => {
  ...
  return (
    <>
      <Button size='sm' className='w-full' color='red' skeleton onClick={showModal}>
        <span className='text-[12px]'>{t("cancel")}</span>
      </Button>
      <Modal title={t("cancel_code")} isModalOpen={isModalOpen} onCancel={handleCancel}>
        {loading ? (
          <Loader />
        ) : (
          <div>
            ...
            <div className='flex flex-row gap-5'>
              <Button className='w-full' size='lg' onClick={onBtnCancel}>
                {t("confirm")}
              </Button>
              <Button skeleton color='blue' className='w-full' size='lg' onClick={handleCancel}>
                {t("cancel")}
              </Button>
            </div>
          </div>
