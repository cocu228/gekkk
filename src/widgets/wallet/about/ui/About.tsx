  ...
  return (
    <div className=' bg-white rounded-md px-6 py-3 mt-3'>
      <div className='flex mt-1 mb-3 items-center'>
        ...
        <h1 className='font-bold text-base text-gray-600'>{name}</h1>
      </div>
      ...
      {isEUR ? null : (
        <div className={`w-full flex flex-row justify-center gap-5 mt-10`}>
          <Button color='blue' className='w-full' onClick={() => navigate(`/exchange?to=${$const}`)}>
            {t("buy")}
          </Button>

          <Button color='gray' className='w-full' onClick={() => navigate(`/exchange?from=${$const}`)}>
            {t("sell")}
          </Button>
        </div>
      )}
