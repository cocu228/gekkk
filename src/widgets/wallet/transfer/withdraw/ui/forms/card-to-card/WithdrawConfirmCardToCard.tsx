...
const WithdrawConfirmCardToCard = ({ amount, comment, cardNumber, selectedCard, cardholderName, handleCancel }) => {
    ...
    <div>
      ...
      <div className={loading ? "collapse" : ""}>
        ...
        <div className={styles.ModalRows}>
          ...
          {cards?.filter(c => c.cardStatus === "ACTIVE")[0] && (
            <>
              {" "}
              <div className='row'>
                <div className='col'>
              ...
              <div className='row mb-1'>
                <div className='col text-[#3A5E66] font-semibold '>
                  <span className={`${styles.ModalRowsValue} break-keep text-nowrap text-ellipsis`}>
                    {formatCardNumber(`${cards?.filter(c => c.cardStatus === "ACTIVE")[0].displayPan}`)}
                  </span>
                </div>
              </div>{" "}
            </>
        ...
        <Form onSubmit={onConfirm}>
          <div className='row my-5'>
            <div className={styles.ButtonContainer}>
              <Button htmlType={"submit"} className={styles.ButtonTwo} disabled={!totalCommission}>
                {t("confirm")}
              </Button>
              <Button skeleton onClick={handleCancel} className={styles.ButtonTwo}>
                {t("cancel")}
              </Button>
            </div>
          </div>
