...
const ExchangeProvider = ({ children, from, to, roomInfo }: IProps) => {
  ...
  const calculatePrice = (from: number, to: number, isSwapped: boolean) => {
    ...
    const result = isSwapped ? new Decimal(from).div(to) : new Decimal(to).div(from);

    return !result.isFinite() || result.isNaN() || result.isZero()
      ? null
  ...
  const calculateToAmount = (from: number, price: number, isSwapped: boolean) => {
    ...
    const result = isSwapped ? new Decimal(from).div(price) : new Decimal(price).mul(from);
  ...
  const handlePriceCurrenciesSwap = () => {
    setState(prev => ({
      ...
      price: {
        ...
        amount: calculatePrice(+prev.from.amount, +prev.to.amount, !prev.price.isSwapped)
      }
    }));
  };
