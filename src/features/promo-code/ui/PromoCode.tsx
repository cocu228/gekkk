      ...
      <div className='px-3 md:pb-0'>
        <form onSubmit={onSubmit}>
          <div className={`flex gap-2 wrapper my-6`}>
            ...
            <span className='text-[14px] text-[var(--gek-dark-grey)] font-normal'>{t("this_code_can_be_used")}</span>
          </div>
          ...
          <div className={"flex justify-evenly md:gap-2"}>
            <Button htmlType='submit' className='w-full' disabled={valInput === "" || loading || isCodeApplied}>
              {t("confirm")}
            </Button>
            <Button skeleton className='w-full' onClick={handleCancel}>
              {t("cancel")}
            </Button>
          </div>
        </form>
