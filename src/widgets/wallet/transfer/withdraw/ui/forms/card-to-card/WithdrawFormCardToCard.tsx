...
const WithdrawFormCardToCard = () => {
  ...
  useEffect(() => {
    setInputs(() => ({
      ...inputs,
      selectedCard: cards?.find(c => ["ACTIVE", "PLASTIC_IN_WAY"].includes(c.cardStatus)) ? cards[0].cardId : null
    }));
  }, [cards]);
  ...
  return !cards ? (
    ...
    <div className='wrapper'>
        ...
        <div className={style.AmountInputWrap}>
          ...
          >
            <InputCurrency.PercentSelector
              currency={currency}
              header={<span className={`${style.InputTitle} mb-[0]`}>{t("amount")}:</span>}
      ...
      <div className='row mb-[45px] w-full md:mb-[10px]'>
        <span className={style.InputTitle}>{t("from_card")}:</span>
      ...
      <div className='mb-[10px]'>
        <span className={style.InputTitle}>{t("to_card")}:</span>
        <Input
      ...
      <div className='row mb-[10px] w-full'>
        <span className={style.InputTitle}>{t("cardholder")}:</span>
        <Input
      ...
      <div className='row w-full'>
        <span className={style.InputTitle}>{t("description")}:</span>
        <Input
          allowDigits
          allowSymbols
      ...
      <Modal title={t("confirm_transaction")} onCancel={handleCancel} isModalOpen={isModalOpen}>
      ...
      <div className='row w-full mb-[10px]'>
        <div className={styles.ButtonContainerCenter}>
          <Button size='lg' className='w-full' onClick={showModal} disabled={!isValidated || inputCurrValid.value}>
            {t("transfer")}
          </Button>
        </div>
      </div>
