...
const MenuItem = ({
  ...
  return (
    <div onClick={onClick} data-item={dataItem} className={`${styles.MenuItem} ${className}`}>
      <div className='flex justify-between'>
        <div className={styles.MenuItemColumn}>
          <div className='font-bold'>{leftPrimary}</div>

          {!leftSecondary ? null : <div className='text-[var(--gek-mid-grey)]'>{leftSecondary}</div>}
        </div>

        <div className={styles.MenuItemColumn}>
          <div className='font-bold text-right'>{rightPrimary}</div>

          {!rightSecondary ? null : <div className='text-[var(--gek-mid-grey)] text-right'>{rightSecondary}</div>}
        </div>
      </div>

      {progress !== null && (
        <div className={`${styles.ProgressBar} mt-2`}>
          <div className={`${styles.CurrentProgress}`} ref={bar} />
        </div>
      )}
    </div>
  );
