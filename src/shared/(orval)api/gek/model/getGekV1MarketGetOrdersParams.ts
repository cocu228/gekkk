 ...
 */
import type { OrderEnumState } from "./orderEnumState";

export type GetGekV1MarketGetOrdersParams = {
  /**
   * Валюта-источник, если нет, то все
   */
  currency_from?: string;
  /**
   * Валюта-цель, если нет, то все
   */
  currency_to?: string;
  /**
   * Начальная дата поиска UTC, если не задано - то 90 дней от end или от from_order_id, игнорирует время
   */
  start?: string;
  /**
   * Конечная дата выборки UTC, если не задано то текущая, игнорирует время
   */
  end?: string;
  /**
   * С какого идентификатора начать выборку (для ленивой загрузки)
   */
  from_order_id?: number;
  /**
   * Идентификатор комнаты обмена. По-умолчанию 0 - ордера из общей комнаты
   */
  room_key?: number;
  /**
   * Количество транзакций в выдаче, по умолчанию 10
   */
  limit?: number;
  /**
   * Статус ордеров, которые требуется отобразить
   */
  ord_states?: OrderEnumState[];
};
