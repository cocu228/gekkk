      ...
      <div className='flex flex-col lg:flex-row lg:justify-between items-center gap-2 lg:w-full'>
        <div className='flex gap-1'>
          ...
          <div data-text={"Update"} className='ellipsis'>
            <span
              className={`cursor-pointer ${styles.UpdateBtn} ${isActiveClass(active)}`}
              onClick={onClick}
              data-testid='Updater'
            >
        ...
        <div
          onClick={onIconClick}
          className='inline-flex items-center gap-2 hover:cursor-pointer hover:text-blue-400 hover:fill-blue-400'
        >
          <span className='font-medium text-3xl leading-7 sm:text-xl'>{quantity}</span>
          <IconApp color='inherit' code='t63' size={22} />
        </div>
      </div>
      <button className='font-medium text-secondary hover:text-blue-400' onClick={onLeave}>
