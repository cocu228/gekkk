    ...
    <>
        ...
        <>
          <div className={styles.CardInfo}>
            {cardsLoading ? (
              ...
              <Carousel>
                <div onClick={() => navigate(md ? "/card-menu" : "/wallet?currency=EUR&tab=bank_cards")}>
                  <NewBankCard />
                </div>
              </Carousel>
            ) : (
              <BankCardsCarousel
                cards={activeCards}
                onItemClick={() => navigate(md ? "card-menu" : "/wallet?currency=EUR&tab=bank_cards")}
              />
            )}
          </div>
      ...
      {!secondaryWallets.length
        ...
        : (isGekkard
            ...
            : helperFilterListGekkoin(secondaryWallets)
          ).map(item => <TokenBar navLinkEvent={NavLinkEvent} curActive={currency} item={item} key={item.id} />)}
      <div className={styles.CurTypeTxtWrap}>
        <div className={styles.TotalBal}>
          <span>{t("total_balance")}</span>
          <span className={styles.TotalBalSum} data-testid='TotalAmount'>
            {toLocaleFiatRounding(totalAmount) ?? "..."}
          </span>
        </div>
      </div>
    </>
