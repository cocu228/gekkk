...
const MenuForm = ({ cardsStorage, onSelectCard, setCardsStorage, setIsNewCardOpened }: IParams) => {
  ...
  const updateCard = (card: ICardData) => {
    setCardsStorage({
      cards: [card, ...cardsStorage.cards.filter(c => c.cardId !== card.cardId)],
      refreshKey: randomId()
    });
  ...
  if (selectedItem === "showData") {
    return <CardInfo cardId={selectedCard.cardId} onBack={() => setSelectedItem(null)} />;
  }
  ...
  return (
    <div className='px-2'>
      ...
      <MenuItem
        ...
        rightPrimary={<Switch onChange={null} defaultCheked={switchChecked} className='pointer-events-none' />}
      />
      ...
      <MenuItem
        ...
        onClick={() => {
          if (md) {
            ...
            setGlobalModal({
              title: t("card_info"),
              content: <CardInfo cardId={selectedCard.cardId} onBack={onGlobalCancel} />
            });
          }
        }}
      />
      ...
      <div className={styles.FooterContainer}>
        {selectedCard?.cardStatus !== "ACTIVE" ? null : (
          <Button
            color='red'
            onClick={() => {
              setSelectedItem(selectedCard?.cardStatus !== "LOCKED" ? "blockCard" : "unblockCard");
            }}
          >
            <IconApp code='t54' size={10} color='#fff' />
            <div>{selectedCard?.cardStatus === "ACTIVE" ? t("block_card") : t("unblock_card")}</div>
          </Button>
        )}
