...
export function ValidateNewCardState(state: INewCardState): boolean {
  const { city, street, postalCode, cardType, countryCode, linkedPhone, cardholderName } = state;

  const requiredFields = [linkedPhone, cardholderName];

  if (cardType === "PLASTIC") {
    requiredFields.push(countryCode, city, postalCode, street);
  }
...
export function ValidateOrderCardState(state: IOrderCardState): boolean {
  const { city, street, postalCode, countryCode } = state;
