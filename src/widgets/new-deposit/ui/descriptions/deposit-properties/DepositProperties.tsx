    ...
    <div className={`${className} -mt-10`}>
      <div className={`wrapper ${styles.InvestBlock}`}>
        <p className='text-lg font-bold mb-5'>
          You invest {amount} EURG {type === DepositType.STRUCTED && `in ${tokenData.name} (${tokenData.$const}) `} for{" "}
          {type === DepositType.FIXED ? 360 : term} days
        </p>
        ...
        <div className='flex flex-col gap-3 md:gap-2'>
          ...
          <InlineProperty
            left='Risk level'
            right={type === DepositType.FIXED ? "Fixed rate deposit" : `${structuredStrategy.name} strategy`}
          />

          <InlineProperty
            left='Returns rate'
            // rightClassName={type === DepositType.STRUCTED ? '' : ''}
            right={type === DepositType.FIXED ? `${isGke ? "1,6" : "0,8"}% per month` : `${risePercent}/${dropPercent}`}
          />
        </div>
      </div>
    </div>
