  ...
  return (
    <Container ref={containerRef} onClick={onClick} className='fade-animation'>
      ...
      <ContentContainer>
        ...
        <TextContainer>
          <NameContainer>
            <Name titleTextColor={"titleTextColor"} unread={unread}>
              {title}
            </Name>

            <Timestamp unread={unread} color={"contentTextColor"}>
              {dateSent}
            </Timestamp>
          </NameContainer>

          <MessageComponent color={"contentTextColor"} width={containerWidth - 96} unread={unread}>
            {lastMessage?.user.id === currentUserId ? "You" : lastMessage?.user.name}:{"  "}
            {lastMessage?.media ? (
              <MediaContainer>
                <MediaIconContainer>{getMediaIcon()}</MediaIconContainer>
                {getMediaText()}
              </MediaContainer>
            ) : (
              <div dangerouslySetInnerHTML={{ __html: lastMessage?.text || "" }}></div>
            )}
          </MessageComponent>
        </TextContainer>
      </ContentContainer>
