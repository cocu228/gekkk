 ...
 */
import type { TransactTypeEnum } from "./transactTypeEnum";

export type GetGekV1WalletGetHistoryTransactionsParams = {
  /**
   * Начальная дата(без времени) поиска UTC, если не задано - то 90 дней от end
   */
  start?: string;
  /**
   * Конечная дата(без времени) выборки UTC, если не задано - то текущая
   */
  end?: string;
  /**
   * После какого идентификатора(в глубь истории) продолжить выборку (для ленивой загрузки), end не учитывается, передается из последнего элемента в выборке
   */
  next_key?: string;
  /**
   * Фильтр по валютам операции
   */
  currencies?: string[];
  /**
   * Фильтр по типам транзакции в выборке
   */
  tx_types?: TransactTypeEnum[];
  /**
   * Количество транзакций в выдаче, по умолчанию 10, но не более 100
   */
  limit?: number;
  /**
   * Загружать дополнительно фиатные транзакции, по умолчанию false - используется если не заданы валюты и нужно загрузить вместе и фиатные и крипто транзакции
   */
  include_fiat?: boolean;
  /**
   * Фильтр криптотранзакций с типом Income и без информации о партнере. При true значения в tx_types будут проигнорированы />
   */
