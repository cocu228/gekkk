...
import { ChatConfig, MessageFormValues } from "../../model/types";
...
const Chat = ({ chatConfig }: IParams) => {
  ...
  const handleSendMessage = async (values: MessageFormValues, form: FormInstance<any>) => {
    ...
    await apiPostMessage(sessionId, "raw", values.message).then(() => {
      form.resetFields();
    });
  };

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const handleSendFile = async (options: {
    file: File;
              ...
              {messages.length
                ? messages.map((message, i, arr) => (
                    <Fragment key={message.id}>
                      {(i === 0 ||
                        timestampToDayYear(message.createdAt) !== timestampToDayYear(arr[i - 1]?.createdAt)) && (
