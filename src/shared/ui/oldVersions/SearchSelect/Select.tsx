...
export const Select: FC<SelectProps> = ({ disabledCurrencies, placeholderText, list, onSelect, isToken, tokenId }) => {
    ...
    <div ref={bodyRef} className={`${style.SelectWrap} ${active && style.SelectActive}`}>
      <div className={style.SelectedBody} onClick={() => setActive(!active)}>
        ...
        {selected && inpValue.length === 0 ? (
          <span className={`${style.SelectedTitle} ${inpValue.length >= 1 && style.SelectedTitleDisabled}`}>
            {selected}
          </span>
        ) : (
        ...
        <input
          ...
          className={`${style.SelectInput} ${isToken && selected && style.SelectInputActiveToken} ${
            selected && style.SelectInputActive
          }`}
        />
      </div>
