import { useContext, useEffect, useMemo, useRef, useState, MouseEvent } from "react";
import { useTranslation } from "react-i18next";

import RefreshButton from "@/shared/ui/refresh-button";
import { IHeaderMenuList, TPropsHeaderMenu } from "@/widgets/header/model/types";
...
// TODO: visual fixes
const HeaderMenu = ({ children, items, className = "", actions, onStateChange = () => {} }: TPropsHeaderMenu) => {
  ...
  const { onOpen, onAction } = useMemo(() => new DropdownMenuFunctions(ref, toggleActive, actions), [ref]);

  const handleOnAction = (action: IHeaderMenuList["action"]) => () => {
    onAction(action);
  };

  const handleOnCalloutFunc = (e: MouseEvent<HTMLSpanElement>) => {
    e.preventDefault();
    e.stopPropagation();
    getAccounts(true);
  };
  ...
  return (
    <div ref={ref} onClick={onOpen} className={`${className} flex items-center cursor-pointer h-full`}>
      <div
        className={`wrapper relative ${!md && "md:pl-14 pl-7"}   md:pr-0  pr-7 min-w-[250px] ${
          isActive ? "active" : ""
        }`}
      >
        {children}
