...
function DepositStats({ days, isGke, amount, isClosed, startingRate, strategyData, linkedCurrency }: IDepositStats) {
  const initialState = {
    rate: null,
    income: null,
    rateDiff: null,
  ...
  const { risePercentage, dropPercentage } = strategyData.percentageType;
  ...
  useEffect(() => {
    ...
    const diffPerc = diff >= 0 ? (max / min) * 100 - 100 : ((max - min) / max) * -100;
    ...
    setState(prev => ({
      ...
      annualProfit: annual,
      income: getDepositCurrentProfit(annual, new Decimal(amount), new Decimal(diffPerc), strategyData, isGke)
    }));
  }, [startingRate, state.rate]);

  return (
    <div
      className={`wrapper flex justify-between gap-4 rounded p-4 pr-12 flex-wrap xxxl:pr-4 pb-8 mb-10 bg-gray-100 w-full ${
        isClosed ? "grayscale" : ""
      } md:flex-col md:gap-8 md:order-1`}
    >
      <div>
      ...
      <div>
        <div className='flex gap-3 items-center mb-3 xxxl:mb-1'>
          ...
          <p className='text-gray-400 font-medium xxxl:text-sm'>{isClosed ? "Closing" : "Current"} rate</p>
        </div>
        {!state.rate ? (
          <SkeletonInput className='h-[22px]' />
        ) : (
          <p className='font-bold text-lg'>
            1 {linkedCurrency} ~ {getCurrencyRounding(state.rate)} €
          </p>
        )}
      </div>
      ...
      <div>
        ...
        {!state.rate ? (
          ...
          <p className='font-bold text-lg'>
            {+state.rateDiff.toFixed(2)} €{" "}
            <span className={state.rateDiff >= 0 ? "text-green" : "text-red-800"}>
      ...
      <div>
        ...
        {!state.rate ? (
          ...
          <p className='font-bold text-sm'>
            ...
            {state.rateDiff > 0 && state.annualProfit !== state.income
              ? `${risePercentage}% from different rates`
              : dropPercentage > 0
              ? `${dropPercentage}% per annum`
              : `${dropPercentage}% from different rates`}
          </p>
        )}
      </div>
    </div>
