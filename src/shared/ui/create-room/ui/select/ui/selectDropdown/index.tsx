...
const SelectDropdown: FC<ISelectDropdownProps> = ({ isOpen, value, options, onChange, onToggleIsOpen }) => {
    ...
    >
      {options.length ? (
        <div ref={dropdownRef} className={`${styles.SelectDropdownContainerInner}`}>
          {options.map(option => (
            <div key={option.name} data-active={getActiveOption(option)} onClick={handleOnChange(option)}>
        ...
        <div className={styles.SelectDropdownContainerNoData}>{t("select_token_no_data")}</div>
      )}
    </div>
  );
};
