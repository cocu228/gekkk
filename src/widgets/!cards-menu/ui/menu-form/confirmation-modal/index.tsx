      ...
      case "monthlyLimit":
        apiSetLimits(
          ...
          updateCard({
            ...selectedCard,
            limits: [
              ...selectedCard.limits.filter(l => l.period !== (action === "dailyLimit" ? "DAILY" : "MONTHLY")),
              {
                type: "ALL",
    ...
    <Modal onCancel={handleCancel} isModalOpen={isModalOpen} title={t("confirm_action")}>
      {loading ? (
        ...
        <>
          ...
          {selectedItem === "unblockCard" && <div className='mb-5'>{t("unblock_selected_bank_card")}</div>}
          ...
          {selectedItem === "disableLimits" && <div className='mb-5'>{t("disable_limits")}</div>}
          ...
          >
            <div className={styles.FormBody}>
              <Button htmlType='submit' text={t("confirm")} className='w-full' />

              <Button skeleton text={t("cancel")} className='w-full' onClick={handleCancel} />
            </div>
          </form>
        </>
      )}
