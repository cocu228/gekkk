...
import { SelectArea } from "./model/types";

const areaMap: Record<SelectArea, ReactNode> = {
...
export function Settings() {
  ...
  const selectedArea = useMemo(() => searchParams.get("sessionsSection") as SelectArea | null, [searchParams]);
  ...
  return (
    <settingsContext.Provider value={{ closeArea: useCallback(() => setSelectedArea(null), []) }}>
      {!md && <h1 className={styles.title}>{t("my_settings")}</h1>}
      <div
        className={`
      ...
      >
        {(!xl || !area) && (
          <div className={`${styles.boxWrap} ${selectedArea && styles.boxWrapMin}`}>
            ...
            <div className={styles.box}>
              <h2
                className={`${styles.accessTitle} ${!md && styles.accessTitleColor} ${md && styles.accessTitleWeight} ${
                  md && styles.accessTitleSize
                }`}
              >
            ...
            {IS_GEKKARD_APP() && (
              <div className={styles.box}>
                <h2
                  className={`${styles.accessTitle} ${!md && styles.accessTitleColor} ${
                    md && styles.accessTitleWeight
                  } ${md && styles.accessTitleSize}`}
                >
