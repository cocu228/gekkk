...
const DateInMessage: FC<IDateInMessageProps> = ({ messages, index, date }) => {
  ...
  const currentMessage = messages[index]?.createdAt?.toLocaleDateString(undefined, {
    day: "numeric",
    month: "short",
    year: "numeric"
  });
  const lastMessage = messages[index - 1]?.createdAt?.toLocaleDateString(undefined, {
    day: "numeric",
    month: "short",
    year: "numeric"
  });

  return !doesPrevDateTimeExist ? (
    <Container data-popupInMessages={true}>
      <DateContainer>
        <DateText>{formatedDate(date)}</DateText>
      </DateContainer>
    </Container>
  ) : (
