...
const ExchangeBar = ({ NavLinkEvent }: IParams) => {
  ...
  return (
    <>
      {/* Exchange page link */}
      <NavLink className={!currencies ? "disabled" : ""} onClick={NavLinkEvent} to={!currencies ? "" : "exchange"}>
        <div className={styles.ItemExchange}>
          <div className='absolute self-center place-self-center'>
            <span className={styles.NavName}>{t("exchange.title")}</span>
          </div>
                ...
                <div className='col flex items-center justify-center flex-col pl-6'>
                  <div className='flex w-full row mb-1 justify-between'>
                    <div className={styles.RoomName}>
                      {!xxxl ? `${item.currency1} - ${item.currency2}` : `${item.currency1} - ${item.currency2}`}
                    </div>
                    <div
                    ...
                    >
                      <IconApp
                        ...
                        code='t69'
                        size={16}
                        className='text-[12px] group-hover:stroke-red-500'
                      />
                    </div>
                  </div>
                    ...
                    <div className='flex row w-full justify-between'>
                      ...
                      >
                        <IconApp
                          ...
                          code='t63'
                          size={16}
                          className='text-[12px] group-hover:stroke-blue-400'
                        />
                      </div>
                    </div>
                  )}
      ...
      <Modal isModalOpen={roomInfoModal.isModalOpen} onCancel={roomInfoModal.handleCancel} title={t("invite_link")}>
        <InviteLink onClose={roomInfoModal.handleCancel} roomInfo={selectedRoom} />
      </Modal>

      <Modal isModalOpen={roomCloseModal.isModalOpen} onCancel={roomCloseModal.handleCancel} title={t("invite_link")}>
