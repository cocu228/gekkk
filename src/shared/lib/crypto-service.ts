...
export async function getTransactionSignParams() {
  const { publicKey: localPublicKey, privateKey: localPrivateKey } = getLocalKeypair();
  const cryptoConfiguration = await getCryptoConfiguration(localPublicKey);

  const decrAppPass = decryptRsaMessage(cryptoConfiguration.secretePassword, localPrivateKey);
...
function getAppUuid(cryptoConfiguration: IAppRegistration, decrAppPass: string) {
  const { salt, appUuid } = cryptoConfiguration;
...
async function getCryptoConfiguration(localPublicKey: string): Promise<IAppRegistration> {
  ...
  const encryptedLocalPublicKey = encryptRsaMessage(serverPublicKey, localPublicKey);

  if (!encryptedLocalPublicKey) return;

  const { data: appRegistration } = await apiRegisterApp(encryptedLocalPublicKey);
...
function getLocalKeypair(): {
  ...
  const privateKey = encrypt
    .getPrivateKey()
    .replace("-----BEGIN RSA PRIVATE KEY-----\n", "")
    .replace("-----END RSA PRIVATE KEY-----", "")
    .replace(/\r?\n|\r/g, "");

  const publicKey = encrypt
    .getPublicKey()
...
function base64url(source: any): string {
  const encodedSource = enc.Base64.stringify(source);
