...
/**
 * Входные параметры для создания заявки на вывод средств
 */
export interface CreateWithdrawIn {
  /**
   * Target address
   * @minLength 1
   */
  ...
  amount: number;
  /** Указывает что делать если адрес вывода является внутренним.
Если false(значение по-умолчанию) - то действие вызовет ошибку
Если true - то перевод будет преобразован во внутренний и выполнен без внешних сетей прямым внутренним переводом */
  auto_inner_transfer?: boolean;
  /** Уникальный идентификатор заявки вывода, не должен совпадать с отправленными для исключения повторов.
...
Unique transaction identifier. ⚠ Note that you should generate new unique id for each withdrawal request. */
   ...
   */
  currency: string;
  /** Withdraw fee.Комиссию нужно присылать отдельно для того чтобы, в случае ее внезапного изменения предупредить или отменить вывод. */
  fee: number;
  /**
   * Memo поле если нужно для текущей валюты
   * @nullable
   */
  memo?: string | null;
  /**
   * Обязательное поле для AML, информация об отправителе-получателе латиницей(для требований регуляторов)
   * @minLength 1
   */
  partner_info: string;
  /**
  ...
  tag?: string | null;
  /**
   * Внутренний ID сети через которую будет идти вывод, если не задано то сеть по-умолчанию
   * @nullable
   */
  token_network?: number | null;
}
