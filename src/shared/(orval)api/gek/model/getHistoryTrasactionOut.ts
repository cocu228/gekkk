 ...
 */
import type { TransactStateEnum } from "./transactStateEnum";
import type { TransactTypeEnum } from "./transactTypeEnum";

export interface GetHistoryTrasactionOut {
  /**
   * Сумма операции
   * @nullable
   */
  amount?: string | null;
  /** Баланс после завершения операции (полный баланс, включая free+orders+out (без in)) */
  balance?: number;
  /**
   * Название валюты операции
   * @nullable
   */
  currency?: string | null;
  /** Дата создания/изменения по транзакции UTC */
  datetime?: string;
  /**
   * Комиссия, которую заплатил клиент
   * @nullable
   */
  fee?: string | null;
  /**
   * ID транзакции внутренний
   * @nullable
   */
  id_transaction?: string | null;
  /** Флаг указывающий (нужно при внутреннем переводе) транзакция направлена к клиенту или от него */
  is_income?: boolean;
  /**
   * Текстовый ключ массива данных, содержащий информацию о границах выборки, нужно для ленивой загрузки следующей порции
   * @nullable
   */
  ...
  /**
   * Описание, комментарий, сообщение или дополнительная информация
   * @nullable
   */
  tag?: string | null;
  tx_type?: TransactTypeEnum;
  /**
   * Краткое описание типа транзакции
   * @nullable
   */
