...
const TermChoose = () => {
  ...
  return (
    <div className='px-10 md:px-3 my-5 md:my-3'>
      <p className='text-gray-400 mb-2 font-medium text-base md:text-sm'>Choose the deposit term (in days)</p>

      <div className='flex'>
        {[90, 180, 270, 360].map(t => (
          <ChooseButton key={`TERM_IN_DAYS_${t}`} isSelected={t === term} onClick={() => onTermChange(t)}>
            {t} days
          </ChooseButton>
        ))}
      </div>
