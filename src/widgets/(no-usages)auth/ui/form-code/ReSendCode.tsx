...
export const ReSendCode = memo(({ isUAS }: { isUAS: boolean }) => {
  const [{ phone }, setSessionAuth] = useSessionStorage<TSessionAuth>("session-auth", {
    phone: "",
    verificationId: "",
    sessionIdUAS: ""
  });
  ...
  return (
    <>
      ...
      <button type='button' disabled={!!state} className='second_value-button'>
