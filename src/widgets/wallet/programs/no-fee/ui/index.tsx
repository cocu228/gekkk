    ...
    <>
      <div className='row mb-10'>
        <div className='col'>
          <div className='info-box-warning'>
            <p className='font-extrabold text-sm mb-3'>{t("locking_tokens_gives", { currency: currency.$const })}</p>

            <p className='text-sm'>
              {t("up_amount_not_exceeding_similar", { currency: currency.$const })}{" "}
              {t("funds_are_blocked_for", { days: 90 })}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className='col bg-[#F9F9FA] px-6 py-5'>
          <div className='row flex flex-wrap mb-4'>
            <div className='col w-1/3  flex flex-row items-center justify-center'>
              <img className='mr-2' width={24} height={24} alt='replenishment' src='/img/icon/replenishment.svg' />
          ...
          <div className='row flex gap-1'>
            <div className='col w-1/3  flex flex-row items-center gap-1 justify-center'>
              ...
              <span className='text-lg text-green font-bold'>
                {!inputCurr.value.string ? null : `(+${inputCurr.value.string})`}
              </span>
            </div>
            ...
            <div className='col w-1/3  flex flex-row items-center justify-center'>
              <span className='text-lg font-bold text-green'>
                {investment ? investment.amount - investment.cur_amount : 0}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className='row mb-4'>
        <div className='flex justify-center col'>
          <Button size='lg' disabled={inputCurrValid.value} onClick={lockConfirmModal.showModal} className='w-full'>
          ...
          <div className='col'>
            <InlineProperty
              ...
              right={
                <>
                  {investment?.amount}{" "}
                  {!inputCurr.value.string ? null : <span className='text-green'>+({inputCurr.value.string})</span>}{" "}
                  {currency.$const}
                </>
              }
            />
...
function NoFeeProperties({ endDate, startDate, templateTerm, t }) {
  return (
    <>
      <div className='row mb-2'>
        <div className='col'>
