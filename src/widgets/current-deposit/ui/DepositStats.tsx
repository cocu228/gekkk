...
function DepositStats({
    ...
    useEffect(() => {
        ...
        (async () => {
            const rate = (await apiGetRates({to: 'EUR'})).data.result[linkedCurrency];
    ...
    return (
        <div
            className={`wrapper flex justify-between gap-4 rounded p-4 pr-12 flex-wrap xxxl:pr-4 pb-8 mb-10 bg-gray-100 w-full ${isClosed ? 'grayscale' : ''} md:flex-col md:gap-8 md:order-1`}>
            <div>
                <div className='flex gap-3 items-center mb-3 xxxl:mb-1'>
                    <img width={24} height={24} src="/img/icon/DepositStartingRateIcon.svg"
                         alt="deposit starting rate"/>
            ...
            <div>
                ...
                {!state.rate ? (
                    ...
                    <p className='font-bold text-lg'>
                        {+state.rateDiff.toFixed(2)} € <span
                        className={state.rateDiff >= 0 ? 'text-green' : 'text-red-800'}>
            ...
            <div>
                ...
                {!state.rate ? (
                    ...
                    <p className='font-bold text-sm'>
                        ...
                        </span> {state.rateDiff > 0 && state.annualProfit !== state.income
