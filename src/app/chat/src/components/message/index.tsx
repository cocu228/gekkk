...
const Message: FC<IMessageProps> = ({
  ...
  return (
    <>
      ...
      />
      {type === "outgoing" ? (
        <OutgoingMessage text={text} created_at={created_at} media={media} last={last} single={single} />
      ) : (
        <IncomingMessage text={text} created_at={created_at} media={media} last={last} single={single} user={user} />
      )}
    </>
  );
};
