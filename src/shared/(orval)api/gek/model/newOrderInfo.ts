...
export interface NewOrderInfo {
  /** Уникальный идентификатор заявки, не должен совпадать с отправленными для исключения повторов.
Число не превышающее int64, например миллисекунды таймера
Unique transaction identifier. ⚠ Note that you should generate new unique id for each request. */
  client_nonce?: number;
  /**
   * Amount of currency from в строковом формате
   * @minLength 1
   */
  from_amount: string;
  /**
  ...
  is_limit?: boolean;
  /** Если не смог полностью встать в стакан, то отмена */
  post_only?: boolean;
  /** Ключ приватной комнаты или 0, если общая */
  room_key?: number;
  /**
   * Amount of currency to или null если market в строковом формате
   * @nullable
   */
