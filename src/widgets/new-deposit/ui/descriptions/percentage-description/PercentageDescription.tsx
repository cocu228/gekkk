...
const PercentageDescription = () => {
  const { percentageType, isGkeDeposit: isGke } = useContext(CtxNewDeposit);
  ...
  return !percentageType ? null : (
    <div className='col px-7 mt-[40px] xl:hidden xxl:p-5'>
      <div className='row flex gap-3 mb-2'>
        ...
        <p>
          Get <b>100%</b> of deposit amount plus <b>{risePercent}%</b> of the rate growth if the crypto rises in price
        </p>
      </div>
      <div className='row flex gap-3'>
        <img width={17} height={10} src='/img/icon/RateDropIcon.svg' alt='UserIcon' />
        <p>
          Get <b>{dropPercent >= 0 ? "100%" : `at least ${100 + dropPercent}%`}</b> of deposit amount{" "}
          {dropPercent <= 0 ? null : (
            <span>
