...
import { ArrowContainer, AttachmentContainer, Container, Input, InputContainer } from "./style";

const MessageFooterContainer: FC = () => {
  const theme = useTheme();

  const { config: authConfig, loading: authLoading } = useContext(CtxAuthInfo);

  const { text, onChange, onEnter, onAttach, onSubmit } = useMessage();
